...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::Balance { address } => {
            // Convert `query_balance`'s error to `StdError`
            let balance_result = query_balance(deps, address).map_err(|err| StdError::generic_err(err.to_string()))?;
            to_binary(&balance_result)
        },
